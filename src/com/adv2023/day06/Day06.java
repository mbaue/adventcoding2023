 ...
 * you expected - they're actually toy boats, each with a big button on top. Holding down the button charges the
 * boat, and releasing the button allows the boat to move. Boats move faster if their button was held longer, but
 * time spent holding the button counts against the total race time. You can only hold the button at the start of
...
public class Day06 {
    public static void main(String[] args) {
        ...
        try {
            ...
            String[] timesString = line.replaceAll("\\s+", " ").replace("Time: ", "").trim().split(" ");
            int[] times = new int[4];
            times[0] = Integer.parseInt(timesString[0].trim());
            times[1] = Integer.parseInt(timesString[1].trim());
            times[2] = Integer.parseInt(timesString[2].trim());
            times[3] = Integer.parseInt(timesString[3].trim());
            line = br.readLine();
            String[] distancesString = line.replaceAll("\\s+", " ").replace("Distance:", "").trim().split(" ");
            int[] distances = new int[4];
            distances[0] = Integer.parseInt(distancesString[0].trim());
            distances[1] = Integer.parseInt(distancesString[1].trim());
            distances[2] = Integer.parseInt(distancesString[2].trim());
            distances[3] = Integer.parseInt(distancesString[3].trim());
            int games = 4;
            int result = 1;
            for (int game = 0; game < games; game++) {
                System.out.println("game=" + game + " -----------------------------------------------------------");
                int pocet = 0;
                for (int i = 0; i <= times[game]; i++) {
                    int x = i * (times[game] - i);
                    //System.out.println("i=" + i + ", d= " + x);
                    if (x > distances[game]) {
                        //  System.out.println("pocita se to");
                        pocet++;
                    }
                }
